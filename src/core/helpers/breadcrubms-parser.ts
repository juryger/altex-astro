import { getNavPathResolver } from "./nav-path-resolver";
...
const getBreadcrumbsFromPath = (path: string): Array<BreadcrumbItem> => {
  ...
  console.log(
    "🚀 ~ getBreadcrumbsFromPath ~ path: %s, parsed collection: %o",
    path,
    splitPath
  );
  ...
  const navPathResolver = getNavPathResolver();
  ...
  while (splitPath.length > 0) {
    ...
    const resolvedPathItem = navPathResolver.resolvePathItem(pathItem);
    console.assert(
      resolvedPathItem !== undefined,
      "🚀 ~ getBreadcrumbsFromPath ~ could not resolve path item: %s",
      pathItem
    );
